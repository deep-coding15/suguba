...
export default function Produit() {
  ...
  // ─── Context ─────────────────────────────────────────────────────────────
  // alertBox("success" | "error", msg)
  // setIsOpenFullScreen({ open: true, model: "...", id: "..." })
  const context = useContext(MyContext);

  // ═══════════════════════════════════════════════════════════════════════════
  // 1. CHARGEMENT DES PRODUITS
  //    Route : GET /api/product/produits   (monté sur /api/product dans index.js)
  // ═══════════════════════════════════════════════════════════════════════════
  const fetchProducts = useCallback(async () => {
    setLoading(true);
    try {
      const data = await fetchDataFromApi(`/api/product/produits?page=1&perPage=10000`);
      if (data?.produits) {
        setProducts(data.produits);
        setFilteredProducts(data.produits);

        // Peuplement des listes de filtres depuis les données réelles
        const cats = [...new Map(
          data.produits.map(p => [p.catId, { id: p.catId, name: p.catName }])
        ).values()].filter(c => c.id?.trim());
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // 2. FILTRAGE RÉACTIF (client-side)
  // ═══════════════════════════════════════════════════════════════════════════
  useEffect(() => {
    let result = [...products];

    if (categoryFilterVal) result = result.filter(p => p.catId         === categoryFilterVal);
    if (subCatFilterVal)   result = result.filter(p => p.subCatId      === subCatFilterVal);
    if (subSubFilterVal)   result = result.filter(p => p.thirdsubCatId === subSubFilterVal);
  ...
  // Cascade : réduire sous-cats quand catégorie change
  useEffect(() => {
    if (categoryFilterVal) {
      ...
      // Reset total si on remet "Toutes"
      const subCats = [...new Map(
        products.map(p => [p.subCatId, { id: p.subCatId, name: p.subCat }])
      ).values()].filter(c => c.id?.trim());
      setSubCatList(subCats);
    }
  }, [categoryFilterVal, products]);

  // Cascade : réduire sous-sous-cats quand sous-cat change
  useEffect(() => {
    if (subCatFilterVal) {
      const filtered = [...new Map(
        products
      ...
      const subSubCats = [...new Map(
        products.map(p => [p.thirdsubCatId, { id: p.thirdsubCatId, name: p.thirdsubCat }])
      ).values()].filter(c => c.id?.trim());
      setSubSubCatList(subSubCats);
    }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // 3. SUPPRESSION
  //    Route : DELETE /api/product/suppression-produit/:id
  //    Alerte : context.alertBox("success" | "error", msg)
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await deleteData(`/api/product/suppression-produit/${productToDelete._id}`);
      if (res?.success) {
        context.alertBox("success", "Produit supprimé avec succès");
        // Retire le produit de la liste locale sans recharger
        setProducts(prev => prev.filter(p => p._id !== productToDelete._id));
      } else {
        context.alertBox("error", res?.message || "Erreur lors de la suppression");
      }
    } catch (err) {
      context.alertBox("error", "Erreur réseau");
    } finally {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // 4. MODIFIER
  //    Ouvre le Dialog fullscreen via context (comme AddProduct)
  //    context.setIsOpenFullScreen({ open: true, model: "Modifier un produit", id: product._id })
  //    EditProduct récupère l'id via : context.isOpenFullScreen.id
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // 5. VISUALISER  (à implémenter plus tard)
  //    context.setIsOpenFullScreen({ open: true, model: "Détail produit", id: product._id })
  //    Dans App.jsx ajouter : {isOpenFullScreen?.model === "Détail produit" && <ViewProduct/>}
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  const handleSelectAll = (e)  => {
    e.target.checked ? setSelected(paginatedProducts.map(p => p._id)) : setSelected([]);
  };
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // 7. EXPORT CSV
  //    Si des lignes sont sélectionnées → exporte uniquement celles-ci
  //    Sinon → exporte tous les produits filtrés
  // ═══════════════════════════════════════════════════════════════════════════
  const handleExport = () => {
    ...
    const headers = [
      "Nom", "Marque", "Categorie", "Sous-Categorie", "Sous-Sous-Categorie",
      "Prix", "Ancien Prix", "Stock", "Note", "Ventes", "Remise"
    ];
    const rows = toExport.map(p => [
      `"${p.name}"`, `"${p.brand}"`, `"${p.catName}"`, `"${p.subCat}"`, `"${p.thirdsubCat}"`,
      p.price, p.oldPrice, p.countInStock, p.rating, p.sales || 0, `${p.discount}%`
    ]);
  ...
  const hasActiveFilter = categoryFilterVal || subCatFilterVal || subSubFilterVal || searchQuery;
  ...
  return (
    <>
      ...
      <div className='card !my-4 !pt-5 shadow-md sm:rounded-lg bg-white'>
        ...
        <div className="flex items-end w-full !pl-5 gap-5 flex-wrap !pr-4">
          ...
          <div className="col w-[18%] min-w-[140px]">
            ...
            <Select className="w-full" size="small" value={categoryFilterVal}
          ...
          {/* Sous-catégorie — désactivé tant qu'aucune catégorie choisie */}
          <div className="col w-[18%] min-w-[140px]">
            <h4 className="text-[13px] font-[600] !mb-2">Filtrez Par sous Categorie</h4>
            <Select className="w-full" size="small" value={subCatFilterVal}
          ...
          {/* Sous-sous-catégorie — désactivé tant qu'aucune sous-catégorie choisie */}
          <div className="col w-[18%] min-w-[140px]">
            <h4 className="text-[13px] font-[600] !mb-2">Filtrez Par sous sous Categorie</h4>
            <Select className="w-full" size="small" value={subSubFilterVal}
              ...
              <Table stickyHeader aria-label="table produits">
                ...
                <TableHead>
                  <TableRow>
                    ...
                    {columns.map(col => (
                      <TableCell key={col.id} style={{ minWidth: col.minWidth, fontWeight: 600 }}>
                        {col.label}
                      </TableCell>
                    ))}
                  </TableRow>
                ...
                <TableBody>
                  {paginatedProducts.length === 0 ? (
                    ...
                    paginatedProducts.map((product) => (
                      <TableRow key={product._id} hover selected={isSelected(product._id)}
                        ...
                        {/* Ventes + barre de progression + stock */}
                        <TableCell>
                          ...
                          <p className="text-[11px] text-gray-400 mt-1">
                            Stock:{" "}
                            <span className={`font-[600] ${product.countInStock < 5 ? 'text-red-500' : 'text-gray-700'}`}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1">
                            ...
                            {/* Modifier → Dialog fullscreen (EditProduct récupère l'id via context) */}
                            ...
                            {/* Visualiser → Dialog fullscreen (à implémenter plus tard) */}
      ...
      <Dialog open={deleteDialogOpen} onClose={handleDeleteCancel}>
        ...
        <DialogActions>
          <Button onClick={handleDeleteCancel} className="!capitalize">
            Annuler
          </Button>
